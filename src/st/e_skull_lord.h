...
void EntitySkullLordOutline(Entity* self) {
    ...
    case 1:
        ...
        switch (self->step_s) {
        case 0:
            scale = FLT_TO_I(rcos(self->ext.skullLord.timer) * 64);
            self->scaleY = scale + 480;
            opacity = 0x80;
...
void EntitySkullLordEye(Entity* self) {
    ...
    case 1:
        ...
        offsetY = self->posY.i.hi + FLT_TO_I(rsin(angle));

        prim = self->ext.skullLord.prim;
