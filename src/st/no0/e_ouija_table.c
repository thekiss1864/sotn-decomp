...
static u8 anim_walk[] = {2, 5, 3, 6, 4, 7, 5, 8, 6, 9, 5, 8, 4, 7, 3, 6, 2,
                         5, 3, 4, 4, 3, 5, 2, 6, 1, 5, 2, 4, 3, 3, 4, 0, 0};
...
void EntityOuijaTable(Entity* self) {
    ...
    case OUIJA_TABLE_ATTACK:
        ...
        if (!--self->ext.ouijaTable.timer) {
            ...
            otherEntity->ext.ouijaTable.isThrown = true;
            // Alternate between throwing vase and beaker
            self->ext.ouijaTable.isThrown ^= 1;
        }
        break;
...
void EntityOuijaTableComponent(Entity* self) {
    ...
    case OUIJA_COMPONENT_CHECK_SPAWN:
        if (self->ext.ouijaTable.spawned) {
    ...
    case OUIJA_COMPONENT_INIT_POS:
        ...
        posY += FLT_TO_I(rcos(self->ext.ouijaTable.unk8E) * 8);
        posX += FLT_TO_I(rsin(self->ext.ouijaTable.unk8E) * 18);
        posX -= self->posX.i.hi;
        posY -= self->posY.i.hi;
        angle = ratan2(posY, posX);
        self->velocityX = FLT_TO_I(rcos(angle) * FLT(24));
        self->velocityY = FLT_TO_I(rsin(angle) * FLT(24));
        // Beaker and vase move in opposite directions
        if (self->params) {
            self->ext.ouijaTable.unk8E += 48;
        } else {
            self->ext.ouijaTable.unk8E -= 48;
        }
    ...
    case OUIJA_COMPONENT_SEEK_ATTACK:
        ...
        if (self->ext.ouijaTable.isThrown) {
            if (hitFlags & 0x80) {
                self->ext.ouijaTable.isThrown = false;
            } else if (hitFlags) {
                self->velocityX = 0;
                self->velocityY = 0;
            }
