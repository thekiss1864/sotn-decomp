// SPDX-License-Identifier: AGPL-3.0-or-later
extern EInit g_EInitPlateLord;
extern EInit D_us_80180B90;

static RECT D_us_80181F80[] = {
    {22, 8, 31, 7},
    {20, 9, 28, 8},
};
static s16 D_us_80181F90[][2] = {{0x240, 0x3F0}, {0x110, 0x330}};
static SVECTOR D_us_80181F98 = {0};

static void func_801CD78C(Pos* src, s32 offset, s16 angle, Pos* dst) {
    if (g_CurrentEntity->facingLeft) {
        angle = -angle;
    }
    *dst = *src;

    dst->x.val += -(offset * rsin(angle) * 16);
    dst->y.val += offset * rcos(angle) * 16;
}

static void func_us_801D2424(Pos* arg0, s16 arg1, s16 arg2, Pos* arg3, s16 arg4,
                             s16 arg5, Primitive* prim) {
    prim->x0 = prim->x1 = arg0->x.i.hi;
    prim->y0 = prim->y1 = arg0->y.i.hi;
    prim->x2 = prim->x3 = arg3->x.i.hi;
    prim->y2 = prim->y3 = arg3->y.i.hi;
    if (g_CurrentEntity->facingLeft) {
        prim->x0 += FLT_TO_I(arg2 * rcos(arg1));
        prim->x1 -= FLT_TO_I(arg2 * rcos(arg1));
        prim->x2 += FLT_TO_I(arg5 * rcos(arg4));
        prim->x3 -= FLT_TO_I(arg5 * rcos(arg4));
    } else {
        prim->x0 -= FLT_TO_I(arg2 * rcos(arg1));
        prim->x1 += FLT_TO_I(arg2 * rcos(arg1));
        prim->x2 -= FLT_TO_I(arg5 * rcos(arg4));
        prim->x3 += FLT_TO_I(arg5 * rcos(arg4));
    }
    prim->y0 -= FLT_TO_I(arg2 * rsin(arg1));
    prim->y1 += FLT_TO_I(arg2 * rsin(arg1));
    prim->y2 -= FLT_TO_I(arg5 * rsin(arg4));
    prim->y3 += FLT_TO_I(arg5 * rsin(arg4));
}

static void func_us_801D26CC(unk_PlatelordStruct* arg0) {
    Entity* entity;
    Pos* position;
    Entity* currentEntity;
    Pos* tempPos;
    s32 angle;
    s32 offset;
    RECT* tempRect;

    tempRect = arg0->unk10;
    entity = arg0->unk0;
    position = (Pos*)&entity->posX;
    tempPos = &arg0->unk8;
    currentEntity = g_CurrentEntity;
    angle = arg0->unk6;
    offset = -tempRect->w + 4;
    func_801CD78C(position, offset, angle, tempPos);
    angle = arg0->unk4;
    offset = -tempRect->x;
    func_801CD78C(tempPos, offset, angle, (Pos*)&currentEntity->posX);
}

static void func_us_801D274C(unk_PlatelordStruct* arg0) {
    Entity* entity;
    Entity* currentEntity;
    Pos* position;
    Pos* tempPos;
    s32 angle;
    s32 offset;
    RECT* tempRect;

    tempRect = arg0->unk10;
    currentEntity = g_CurrentEntity;
    tempPos = &arg0->unk8;
    entity = arg0->unk0;
    position = (Pos*)&entity->posX;
    angle = arg0->unk4;
    offset = tempRect->x;
    func_801CD78C((Pos*)&currentEntity->posX, offset, angle, tempPos);
    angle = arg0->unk6;
    offset = tempRect->w - 4;
    func_801CD78C(tempPos, offset, angle, position);
}

static bool func_us_801D27C4(unk_PlatelordStruct* arg0, bool isNegative) {
    RECT* rect;
    bool ret;
    s32 dx;
    s32 dy;
    s32 x;
    s32 w;
    s32 distance;

    Entity* tempEntity;
    Pos* posA;
    Pos* posB;
    Pos* posC;

    posA = (Pos*)&g_CurrentEntity->posX;
    tempEntity = arg0->unk0;
    posB = (Pos*)&tempEntity->posX;
    rect = arg0->unk10;
    posC = &arg0->unk8;

    dx = posB->x.val - posA->x.val;
    if (g_CurrentEntity->facingLeft) {
        dx = -dx;
    }
    dy = posB->y.val - posA->y.val;
    arg0->unk4 = ratan2(-dx, dy);

    x = rect->x * 256;
    w = (rect->w - 4) * 256;
    dx /= 256;
    dy /= 256;
    distance = SquareRoot0((dx * dx) + (dy * dy));
    if (((rect->x + rect->w - 4) * 256) < distance) {
        ret = true;
        distance = (rect->x + rect->w - 4) * 256;
    } else {
        ret = false;
    }
    distance = (distance * x) / (x + w);
    w = (x * x) - (distance * distance);
    w = SquareRoot0(w);
    if (isNegative) {
        arg0->unk4 -= ratan2(w, distance);
    } else {
        arg0->unk4 += ratan2(w, distance);
    }
    func_801CD78C(posA, rect->x, arg0->unk4, posC);

    dx = posB->x.val - posC->x.val;
    if (g_CurrentEntity->facingLeft) {
        dx = -dx;
    }
    dy = posB->y.val - posC->y.val;
    arg0->unk6 = ratan2(-dx, dy);

    return ret;
}

#include "step_towards.h"

void EntityPlateLord(Entity* self) {
    s32 primIndex;
    s32 posY;
    s16* tempPoint16A;
    RECT* tempRect;
    Pos* tempPos;
    Pos pos;
    Collider collider;

    Entity* part;
    Primitive* prim;
    unk_PlatelordStruct* unkStructA;
    unk_PlatelordStruct* unkStructB;
    s32 posX;
    s32 towards;
    s32 i;
    s32 tempS16;
    unk_PlatelordStruct* unkStructC;

    if (self->ext.plateLord.unkB0) {
        self->ext.plateLord.unkB0--;
    } else if (
        ((self->step == 5) || (self->step == 6)) && (self->hitFlags & 3)) {
        PlaySfxPositional(SFX_PLATE_LORD_PAIN);
        self->step_s = 0;
        self->step = 7;
    }
    if ((self->flags & FLAG_DEAD) && (self->step < 0x10)) {
        if ((self->step == 5) || (self->step == 6)) {
            self->ext.plateLord.unk87 = 0;
        } else {
            self->ext.plateLord.unk87 = 1;
        }
        self->step_s = 0;
        self->step = 0x10;
        PlaySfxPositional(SFX_PLATE_LORD_DEATH);
    }
    switch (self->step) {
    case 0:
        InitializeEntity(g_EInitPlateLord);
        self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
        self->animCurFrame = 3;
        self->zPriority = 0xB0;
        self->hitboxWidth = 0xC;
        self->hitboxHeight = 0x14;
        self->hitboxOffY = 0x14;
        primIndex = g_api.AllocPrimitives(PRIM_GT4, 4);
        if (primIndex == -1) {
            DestroyEntity(self);
            return;
        }
        self->flags |= FLAG_HAS_PRIMS;
        self->primIndex = primIndex;
        prim = &g_PrimBuf[primIndex];
        self->ext.prim = prim;

        prim->clut = PAL_PLATE_LORD_UNK2;
        prim->priority = 0xAF;
        setUV4(prim, 0x7A, 0x19, 0x82, 0x22, 0x6D, 0x25, 0x77, 0x2E);
        prim = prim->next;

        prim->clut = PAL_PLATE_LORD_UNK3;
        prim->priority = 0xAF;
        prim->u0 = prim->u2 = 0x24;
        prim->u1 = prim->u3 = 0x34;
        prim->v0 = prim->v1 = 0x39;
        prim->v2 = prim->v3 = 0x57;
        prim = prim->next;

        prim->clut = PAL_PLATE_LORD_UNK2;
        prim->priority = 0xB1;
        prim->u0 = 0x6B;
        prim->u1 = 0x7A;
        prim->u2 = 0x73;
        prim->u3 = 0x82;
        prim->v0 = prim->v1 = 2;
        prim->v2 = prim->v3 = 0xF;
        prim = prim->next;

        prim->clut = PAL_PLATE_LORD_UNK1;
        prim->priority = 0xB2;
        prim->u0 = 0x36;
        prim->u1 = 0x47;
        prim->u2 = 0x3F;
        prim->u3 = 0x4F;
        prim->v0 = prim->v1 = 0x3B;
        prim->v2 = prim->v3 = 0x59;
        prim = prim->next;

        prim = self->ext.prim;
        while (prim != NULL) {
            prim->tpage = 0x13;
            prim->drawMode = DRAW_UNK02;
            prim = prim->next;
        }
        part = self + 1;
        self->ext.plateLord.unk88.unk0 = part;
        self->ext.plateLord.unk88.unk10 = &D_us_80181F80[0];
        CreateEntityFromEntity(E_PLATE_LORD_UNK1, self, part);
        part->params = 0;
        part = self + 2;
        self->ext.plateLord.unk9C.unk0 = part;
        self->ext.plateLord.unk9C.unk10 = &D_us_80181F80[1];

        CreateEntityFromEntity(E_PLATE_LORD_UNK1, self, part);
        part->params = 1;

        part = self + 3;
        CreateEntityFromEntity(E_PLATE_LORD_UNK2, self, part);

        part = self + 4;
        CreateEntityFromEntity(E_PLATE_LORD_UNK3, self, part);
        part->params = 0;

        part = self + 5;
        CreateEntityFromEntity(E_PLATE_LORD_UNK4, self, part);

        part = self + 6;
        CreateEntityFromEntity(E_PLATE_LORD_UNK3, self, part);
        part->params = 1;

        part = self + 7;
        CreateEntityFromEntity(E_PLATE_LORD_UNK3, self, part);
        part->params = 2;

        part = self + 8;
        CreateEntityFromEntity(E_PLATE_LORD_UNK3, self, part);
        part->params = 3;

        part = self + 9;
        CreateEntityFromEntity(E_PLATE_LORD_UNK5, self, part);

        part = self;
        for (i = 0; i < 8; i++, part++) {
            part->nextPart = part + 1;
            part->parent = self;
        }
        part->nextPart = self;
        part->parent = self;
        self->parent = NULL;
        self->ext.plateLord.unk84 = 0;
        self->ext.plateLord.unk9C.unk4 = -0x100;
        self->ext.plateLord.unk9C.unk6 = -0xE0;
        self->ext.plateLord.unk88.unk4 = 0x1C0;
        self->ext.plateLord.unk88.unk6 = -0x40;
    case 1:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        func_us_801D26CC(unkStructA);
        func_us_801D274C(unkStructB);
        self->step = 2;
        break;
    case 2:
    case 3:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        switch (self->step_s) {
        case 0:
            part = unkStructA->unk0;
            posY = part->posY.i.hi - self->posY.i.hi;
            if (posY < 0x2E) {
                self->posY.val -= FIX(0.125);
            } else {
                self->step_s++;
            }
            break;
        case 1:
            part = unkStructA->unk0;
            posY = part->posY.i.hi - self->posY.i.hi;
            if (posY > 0x2B) {
                self->posY.val += FIX(0.125);
            } else {
                self->step_s--;
            }
            break;
        }
        func_us_801D27C4(&self->ext.plateLord.unk88, false);
        func_us_801D27C4(&self->ext.plateLord.unk9C, true);
        if (self->step == 2) {
            towards = 0x80;
        } else {
            towards = 0x100;
        }
        if (GetDistanceToPlayerY() < 0x40) {
            posX = GetDistanceToPlayerX();
            if (posX < towards) {
                self->ext.plateLord.unk80 = 0x80;
                self->step_s = 0;
                if (posX < 0x60) {
                    self->step = 6;
                } else {
                    self->step = 5;
                }
                if (self->facingLeft == (GetSideToPlayer() & 1)) {
                    self->step_s = 0;
                    self->step = 4;
                }
            }
        }
        break;
    case 5:
        part = self + 3;
        StepTowards(&part->ext.plateLordUnknown.unkB0, ROT(174.375), 0x20);
        if (self->ext.plateLord.unk86) {
            part->ext.plateLordUnknown.unk98 -= 4;
            if (part->ext.plateLordUnknown.unk98 < ROT(45)) {
                self->ext.plateLord.unk86 = 0;
            }
        } else {
            part->ext.plateLordUnknown.unk98 += 4;
            if (part->ext.plateLordUnknown.unk98 > ROT(50.625)) {
                self->ext.plateLord.unk86 = 1;
            }
        }
        posX = g_Tilemap.scrollX.i.hi + self->posX.i.hi;
        tempPoint16A = &D_us_80181F90[self->params][0];
        if (self->facingLeft) {
            tempPoint16A++;
            if (*tempPoint16A < posX) {
                self->step_s = 1;
                self->step = 6;
                break;
            }
        } else {
            if (posX < *tempPoint16A) {
                self->step_s = 1;
                self->step = 6;
                break;
            }
        }
        switch (self->step_s) {
        case 0:
            self->ext.plateLord.unk80 = 0x40;
            self->step_s++;
            /* fallthrough */
        case 1:
            if (self->ext.plateLord.unk84 == 0) {
                unkStructA = &self->ext.plateLord.unk9C;
                unkStructB = &self->ext.plateLord.unk88;
            } else {
                unkStructA = &self->ext.plateLord.unk88;
                unkStructB = &self->ext.plateLord.unk9C;
            }
            part = unkStructB->unk0;
            if (part->ext.plateLord.unk85) {
                part = unkStructA->unk0;
                if (!part->ext.plateLord.unk85) {
                    self->ext.plateLord.unk84 ^= 1;
                } else {
                    self->step_s++;
                    break;
                }
            }
            if (self->ext.plateLord.unk84 == 1) {
                StepTowards(&unkStructA->unk4, 0x200, 8);
                StepTowards(&unkStructA->unk6, 0x100, 8);
            }
            StepTowards(&unkStructB->unk4, 0, 0x10);
            StepTowards(&unkStructB->unk6, 0, 0x10);
            func_us_801D26CC(unkStructA);
            func_us_801D274C(unkStructB);
            break;
        case 2:
            self->ext.plateLord.unk84 = 1;
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
            posX = self->posX.val;
            towards = StepTowards(&unkStructA->unk4, 0xE0, 0x18);
            towards += StepTowards(&unkStructA->unk6, -0x100, 0x18);
            func_us_801D26CC(unkStructA);
            posX = self->posX.val - posX;
            part = unkStructB->unk0;
            part->posX.val += posX;
            posX = part->posX.i.hi - self->posX.i.hi;
            if (self->facingLeft) {
                posX = -posX;
            }
            if (posX > 6) {
                if (self->facingLeft) {
                    part->posX.val += FIX(2);
                } else {
                    part->posX.val -= FIX(2);
                }
            } else {
                towards++;
            }
            func_us_801D27C4(unkStructB, true);
            if (towards == 3) {
                PlaySfxPositional(SFX_STOMP_HARD_B);
                self->ext.plateLord.unk84 ^= 1;
                self->step_s++;
            }
            break;
        case 3:
            self->ext.plateLord.unk84 = 0;
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
            posX = self->posX.val;
            towards = StepTowards(&unkStructA->unk4, -0x140, 0x10);
            towards += StepTowards(&unkStructA->unk6, -0x80, 0x10);
            func_us_801D26CC(unkStructA);
            posX = self->posX.val - posX;
            part = unkStructB->unk0;
            part->posX.val += posX;
            posX = self->posX.i.hi - part->posX.i.hi;
            if (self->facingLeft) {
                posX = -posX;
            }
            if (posX < 0x16) {
                if (self->facingLeft) {
                    part->posX.val += FIX(1.5);
                } else {
                    part->posX.val -= FIX(1.5);
                }
            } else {
                towards++;
            }
            func_us_801D27C4(unkStructB, false);
            if (towards == 3) {
                self->ext.plateLord.unk84 ^= 1;
                PlaySfxPositional(SFX_DIPLOCEPHALUS_STOMP);
                self->step_s--;
            }
            break;
        }
        if (self->ext.plateLord.unk80) {
            self->ext.plateLord.unk80--;
        } else {
            self->step_s = 0;
            if (self->facingLeft == (GetSideToPlayer() & 1)) {
                self->step = 4;
            } else {
                self->step = 9;
            }
        }
        break;
    case 6:
        part = self + 3;
        StepTowards(&part->ext.plateLordUnknown.unkB0, ROT(180), 0x20);
        if (self->ext.plateLord.unk86) {
            part->ext.plateLordUnknown.unk98 -= 8;
            if (part->ext.plateLordUnknown.unk98 < ROT(33.75)) {
                self->ext.plateLord.unk86 = 0;
            }
        } else {
            part->ext.plateLordUnknown.unk98 += 8;
            if (part->ext.plateLordUnknown.unk98 > ROT(45)) {
                self->ext.plateLord.unk86 = 1;
            }
        }
        posX = g_Tilemap.scrollX.i.hi + self->posX.i.hi;
        tempPoint16A = &D_us_80181F90[self->params][0];
        if (self->facingLeft) {
            if (posX < *tempPoint16A) {
                self->step_s = 1;
                self->step = 5;
                break;
            }
        } else {
            tempPoint16A++;
            if (*tempPoint16A < posX) {
                self->step_s = 1;
                self->step = 5;
                break;
            }
        }
        switch (self->step_s) {
        case 0:
            self->ext.plateLord.unk80 = 0x40;
            self->step_s++;
            /* fallthrough */
        case 1:
            if (self->ext.plateLord.unk84 == 0) {
                unkStructA = &self->ext.plateLord.unk9C;
                unkStructB = &self->ext.plateLord.unk88;
            } else {
                unkStructA = &self->ext.plateLord.unk88;
                unkStructB = &self->ext.plateLord.unk9C;
            }
            part = unkStructB->unk0;
            if (part->ext.plateLord.unk85) {
                part = unkStructA->unk0;
                if (!part->ext.plateLord.unk85) {
                    self->ext.plateLord.unk84 ^= 1;
                } else {
                    self->step_s++;
                    break;
                }
            }
            if (self->ext.plateLord.unk84 == 1) {
                StepTowards(&unkStructA->unk4, 0x200, 8);
                StepTowards(&unkStructA->unk6, 0x100, 8);
            }
            StepTowards(&unkStructB->unk4, 0, 0x10);
            StepTowards(&unkStructB->unk6, 0, 0x10);
            func_us_801D26CC(unkStructA);
            func_us_801D274C(unkStructB);
            break;
        case 2:
            self->ext.plateLord.unk84 = 0;
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
            posX = self->posX.val;
            towards = StepTowards(&unkStructA->unk4, 0, 0x20);
            towards += StepTowards(&unkStructA->unk6, 0, 0x20);
            func_us_801D26CC(unkStructA);
            posX = self->posX.val - posX;
            part = unkStructB->unk0;
            part->posX.val += posX;
            posX = self->posX.i.hi - part->posX.i.hi;
            if (self->facingLeft) {
                posX = -posX;
            }
            if (posX > 0x10) {
                if (self->facingLeft) {
                    part->posX.val -= FIX(0.75);
                } else {
                    part->posX.val += FIX(0.75);
                }
            } else {
                towards++;
            }
            func_us_801D27C4(unkStructB, false);
            if (towards == 3) {
                PlaySfxPositional(SFX_DIPLOCEPHALUS_STOMP);
                self->step_s++;
            }
            break;
        case 3:
            self->ext.plateLord.unk84 = 1;
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
            posX = self->posX.val;
            towards = StepTowards(&unkStructA->unk4, 0x1C0, 0x18);
            towards += StepTowards(&unkStructA->unk6, 0x180, 0x18);
            func_us_801D26CC(unkStructA);
            posX = self->posX.val - posX;
            part = unkStructB->unk0;
            part->posX.val += posX;
            posX = part->posX.i.hi - self->posX.i.hi;
            if (self->facingLeft) {
                posX = -posX;
            }
            if (posX < 12) {
                if (self->facingLeft) {
                    part->posX.val -= FIX(1);
                } else {
                    part->posX.val += FIX(1);
                }
            } else {
                towards++;
            }
            func_us_801D27C4(unkStructB, true);
            if (towards == 3) {
                PlaySfxPositional(SFX_DIPLOCEPHALUS_STOMP);
                self->step_s--;
            }
            break;
        }
        if (self->ext.plateLord.unk80) {
            self->ext.plateLord.unk80--;
        } else {
            self->step_s = 0;
            if (self->facingLeft == (GetSideToPlayer() & 1)) {
                self->step = 4;
            } else {
                self->step = 9;
            }
        }
        break;
    case 7:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        switch (self->step_s) {
        case 0:
            if (self->facingLeft) {
                self->velocityX = FIX(-0.75);
            } else {
                self->velocityX = FIX(0.75);
            }
            self->velocityY = FIX(0.5);
            self->ext.plateLord.unkB0 = 0x50;
            self->step_s++;
            /* fallthrough */
        case 1:
            self->posX.val += self->velocityX;
            self->posY.val += self->velocityY;
            self->velocityY -= self->velocityY >> 8;
            if (self->facingLeft) {
                self->velocityX += FIX(0.125);
            } else {
                self->velocityX -= FIX(0.125);
            }

            if (abs(self->velocityX) == FIX(0.75)) {
                self->step_s = 0;
                self->step = 6;
            }
            func_us_801D27C4(unkStructA, false);
            func_us_801D27C4(unkStructB, false);
        }
        part = self + 3;
        StepTowards(&part->ext.plateLordUnknown.unkB0, ROT(196.875), 0x30);
        break;
    case 9:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        part = self + 3;
        switch (self->step_s) {
        case 0:
            part = unkStructB->unk0;
            if (part->ext.plateLord.unk85) {
                part = unkStructA->unk0;
                if (part->ext.plateLord.unk85) {
                    part = self + 5;
                    part->step = 8;
                    self->ext.plateLord.unk84 = 0;
                    self->step_s = 1;
                } else {
                    self->ext.plateLord.unk84 ^= 1;
                    self->step_s = 0;
                }
            } else {
                StepTowards(&unkStructB->unk4, 0, 0xC);
                StepTowards(&unkStructB->unk6, 0, 0xC);
                if (self->ext.plateLord.unk84 == 1) {
                    StepTowards(&unkStructA->unk4, 0x200, 8);
                    StepTowards(&unkStructA->unk6, 0x100, 8);
                }
            }
            break;
        case 1:
            towards = 0;
            towards += StepTowards(
                &part->ext.plateLordUnknown.unk80, ROT(191.25), 0x3C);
            towards += StepTowards(
                &part->ext.plateLordUnknown.unk96, ROT(202.5), 0x48);
            towards += StepTowards(
                &part->ext.plateLordUnknown.unk94, ROT(56.25), 0x20);
            StepTowards(&part->ext.plateLordUnknown.unk98, ROT(67.5), 0x20);
            if (towards == 3) {
                self->step_s++;
            }
            break;
        case 2:
            part->ext.plateLordUnknown.unk82 = ROT(270);
            part->ext.plateLordUnknown.unk86 = 0xC0;
            part->step = 9;
            part = self + 5;
            part->step = 9;
            self->step_s++;
            break;
        case 3:
            if (self->ext.plateLord.unk86) {
                part->ext.plateLordUnknown.unk98 -= 2;
                if (part->ext.plateLordUnknown.unk98 < ROT(66.09375)) {
                    self->ext.plateLord.unk86 = 0;
                }
            } else {
                part->ext.plateLordUnknown.unk98 += 2;
                if (part->ext.plateLordUnknown.unk98 > ROT(68.90625)) {
                    self->ext.plateLord.unk86 = 1;
                }
            }
            part = self + 3;
            if (part->ext.plateLordUnknown.unk84 > 0xE8) {
                StepTowards(&unkStructA->unk4, 0x80, 0x18);
                StepTowards(&unkStructA->unk6, 0, 0x18);
                func_us_801D26CC(unkStructA);
                StepTowards(&unkStructB->unk4, 0x340, 0x10);
                StepTowards(&unkStructB->unk6, 0x40, 0x10);
                func_us_801D274C(unkStructB);
            }
            break;
        }
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        func_us_801D26CC(unkStructA);
        func_us_801D274C(unkStructB);
        break;
    case 11:
        unkStructA = &self->ext.plateLord.unk9C;
        unkStructB = &self->ext.plateLord.unk88;
        part = self + 3;
        switch (self->step_s) {
        case 0:
            if (part->ext.plateLordUnknown.unk96 < ROT(112.5)) {
                part->ext.plateLordUnknown.unk94 -= ROT(5.625);
                if (part->ext.plateLordUnknown.unk94 <= ROT(0)) {
                    part->ext.plateLordUnknown.unk94 = ROT(0);
                }
            }
            part->ext.plateLordUnknown.unk96 -= ROT(14.0625);
            if (part->ext.plateLordUnknown.unk96 < ROT(123.75)) {
                part->animCurFrame = 18;
                part->ext.plateLordUnknown.unk80 -= ROT(20.390625);
            }
            if (part->ext.plateLordUnknown.unk80 < ROT(78.75)) {
                self->step_s++;
            }
            StepTowards(&unkStructA->unk4, -0x280, 0x40);
            StepTowards(&unkStructA->unk6, -0x280, 0x40);
            posX = self->posX.val;
            func_us_801D26CC(unkStructA);
            posX = self->posX.val - posX;
            part = unkStructB->unk0;
            if (part->ext.plateLord.unk85) {
                if (!self->ext.plateLord.unk80) {
                    self->ext.plateLord.unk80++;
                    if (self->facingLeft) {
                        EntityGreyPuffSpawner(part, 3, 3, 12, 4, 3, -4);
                    } else {
                        EntityGreyPuffSpawner(part, 3, 3, -12, 4, 3, 4);
                    }
                }
                part->posX.val += posX;
                func_us_801D27C4(unkStructB, false);
            } else {
                func_us_801D274C(unkStructB);
                self->ext.plateLord.unk80 = 0;
            }
            part = self + 3;
            StepTowards(&part->ext.plateLordUnknown.unkB0, ROT(135), 0x30);
            break;
        case 1:
            func_us_801D26CC(unkStructA);
            func_us_801D274C(unkStructB);
            break;
        }
        break;
    case 14:
        unkStructA = &self->ext.plateLord.unk9C;
        unkStructB = &self->ext.plateLord.unk88;
        StepTowards(&unkStructA->unk4, 0, 0x20);
        StepTowards(&unkStructA->unk6, -0x40, 0x20);
        StepTowards(&unkStructB->unk4, 0x100, 0x10);
        StepTowards(&unkStructB->unk6, 0, 0x10);
        part = self + 3;
        StepTowards(&part->ext.plateLordUnknown.unkB0, ROT(174.375), 0x20);
        func_us_801D26CC(unkStructA);
        func_us_801D274C(unkStructB);
        part = self + 3;
        switch (self->step_s) {
        case 0:
            towards =
                StepTowards(&part->ext.plateLordUnknown.unk94, ROT(135), 0x40);
            if ((towards != 0) && part->animCurFrame != 2) {
                part->animCurFrame = 2;
                part->ext.plateLordUnknown.unk94 = ROT(90);
                towards = 0;
            }
            towards +=
                StepTowards(&part->ext.plateLordUnknown.unk96, ROT(135), 0x20);
            if (towards == 2) {
                part->ext.plateLordUnknown.unk94 = ROT(45);
                part->ext.plateLordUnknown.unk96 = ROT(-45);
                self->step_s++;
            }
            break;
        case 1:
            towards = 0;
            towards += StepTowards(
                &part->ext.plateLordUnknown.unk80, ROT(33.75), 0x20);
            towards +=
                StepTowards(&part->ext.plateLordUnknown.unk94, ROT(22.5), 8);
            towards += StepTowards(
                &part->ext.plateLordUnknown.unk96, ROT(-39.375), 0x10);
            StepTowards(&part->ext.plateLordUnknown.unk98, ROT(33.75), 0x10);
            if (towards == 3) {
                self->step = 3;
                self->step_s = 0;
                self->facingLeft = (GetSideToPlayer() & 1) ^ 1;
                part = self + 5;
                part->step = 3;
            }
            break;
        }
        break;
    case 4:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        switch (self->step_s) {
        case 0:
            self->facingLeft ^= 1;
            part = unkStructA->unk0;
            if (self->facingLeft ^ self->ext.plateLord.unk84) {
                part->posX.i.hi -= 0x20;
            } else {
                part->posX.i.hi += 0x20;
            }
            self->ext.plateLord.unk80 = 0x10;
            self->step_s++;
            break;
        case 1:
            if (!--self->ext.plateLord.unk80) {
                self->step_s = 0;
                self->step = 3;
            }
            break;
        }
        func_us_801D26CC(unkStructA);
        func_us_801D274C(unkStructB);
        break;
    case 16:
        if (self->ext.plateLord.unk84 == 0) {
            unkStructA = &self->ext.plateLord.unk9C;
            unkStructB = &self->ext.plateLord.unk88;
        } else {
            unkStructA = &self->ext.plateLord.unk88;
            unkStructB = &self->ext.plateLord.unk9C;
        }
        switch (self->step_s) {
        case 0:
            part = self;
            for (i = 0; i < 10; i++, part++) {
                part->hitboxState = HITBOX_INACTIVE;
            }
            part = self + 3;
            part->step = 3;
            part = self + 5;
            part->step = 3;
            if (self->ext.plateLord.unk87) {
                self->step_s = 3;
                part = self + 5;
                part->step = 0x12;
            } else {
                self->step_s = 3;
                part = self + 5;
                part->step = 0x12;
            }
            /* fallthrough */
        case 1:
            part = self + 3;
            towards =
                StepTowards(&part->ext.plateLordUnknown.unk80, ROT(180), 0xA0);
            towards +=
                StepTowards(&part->ext.plateLordUnknown.unk96, ROT(180), 0xA0);
            towards +=
                StepTowards(&part->ext.plateLordUnknown.unk94, ROT(0), 0x80);
            func_us_801D26CC(unkStructA);
            func_us_801D274C(unkStructB);
            if (towards == 3) {
                part = self + 5;
                part->step = 0x10;
                self->step_s = 2;
            }
            break;
        case 2:
            if (self->ext.plateLord.unk87) {
                self->step_s++;
            }
            func_us_801D26CC(unkStructA);
            func_us_801D274C(unkStructB);
            break;
        case 3:
            self->posY.val += self->velocityY;
            self->velocityY += 0x400;
            posX = self->posX.i.hi;
            posY = self->posY.i.hi + 0x10;
            g_api.CheckCollision(posX, posY, &collider, 0);
            if (collider.effects & EFFECT_SOLID) {
                self->step_s++;
            }
            func_us_801D27C4(unkStructA, false);
            func_us_801D27C4(unkStructB, false);
            break;
        case 4:
            prim = self->ext.prim;
            while (prim != NULL) {
                prim->drawMode = DRAW_HIDE;
                prim = prim->next;
            }
            part = self + 2;
            part->step = 0x10;
            part = self + 1;
            part->step = 0x10;
            self->ext.plateLord.unk80 = 0x30;
            PlaySfxPositional(SFX_FM_STUTTER_EXPLODE);
            self->step_s++;
            break;
        case 5:
            if (!--self->ext.plateLord.unk80) {
                part = self;
                for (i = 0; i < 9; i++, part++) {
                    DestroyEntity(part);
                }
                return;
            }
            if (!(self->ext.plateLord.unk80 & 3)) {
                posX = self->posX.i.hi;
                posY = self->posY.i.hi;
                posX += (Random() & 0x1F) - 0x10;
                posY += (Random() & 0x1F) - 0x10;
                part = AllocEntity(&g_Entities[224], &g_Entities[256]);
                if (part != NULL) {
                    CreateEntityFromCurrentEntity(E_EXPLOSION, part);
                    part->params = 1;
                    part->zPriority = self->zPriority + 8;
                    part->posX.i.hi = posX;
                    part->posY.i.hi = posY;
                }
            }
            break;
        }
        break;
    }
    prim = self->ext.prim;
    unkStructC = &self->ext.plateLord.unk88;
    for (i = 0; i < 2; i++, unkStructC++) {
        part = unkStructC->unk0;
        tempPos = &unkStructC->unk8;
        tempS16 = unkStructC->unk4;
        tempRect = unkStructC->unk10;
        func_us_801D2424((Pos*)&self->posX, tempS16, tempRect->y, tempPos,
                         tempS16, tempRect->y, prim);
        if (self->palette & PAL_UNK_FLAG) {
            prim->clut = self->palette & 0xFFF;
        } else {
            prim->clut = PAL_PLATE_LORD_UNK2;
        }
        prim = prim->next;
        tempS16 = unkStructC->unk6;
        func_801CD78C(tempPos, -4, tempS16, &pos);
        func_us_801D2424(&pos, tempS16, tempRect->h, (Pos*)&part->posX, tempS16,
                         tempRect->h, prim);
        if (self->palette & PAL_UNK_FLAG) {
            prim->clut = self->palette & 0xFFF;
        } else {
            prim->clut = PAL_PLATE_LORD_UNK1;
        }
        prim = prim->next;
    }
}

void func_us_801D4324(Entity* self) {
    Collider collider;
    Entity* tempEntity;
    s32 posX;
    s32 posY;

    if (self->params) {
        tempEntity = self - 2;
    } else {
        tempEntity = self - 1;
    }
    self->facingLeft = tempEntity->facingLeft;
    switch (self->step) {
    case 0:
        InitializeEntity(g_EInitPlateLord);
        self->hitPoints = 0x7FFE;
        self->hitboxWidth = 4;
        self->hitboxHeight = 8;
        if (self->params) {
            self->animCurFrame = 13;
            self->zPriority = 0xB1;
        } else {
            self->animCurFrame = 16;
            self->zPriority = 0xAE;
        }
        break;
    case 1:
        posX = self->posX.i.hi;
        posY = self->posY.i.hi + 4;
        g_api.CheckCollision(posX, posY, &collider, 0);
        if (collider.effects & EFFECT_SOLID) {
            self->posY.i.hi += collider.unk18;
            self->ext.plateLord.unk85 = 1;
            break;
        }
        self->ext.plateLord.unk85 = 0;
        break;
    case 16:
        tempEntity = AllocEntity(&g_Entities[224], &g_Entities[256]);
        if (tempEntity != NULL) {
            CreateEntityFromEntity(E_EXPLOSION, self, tempEntity);
            tempEntity->params = 1;
            tempEntity->zPriority = self->zPriority + 4;
        }
        DestroyEntity(self);
        break;
    }
}

void func_us_801D44A0(Entity* self) {
    long sp70;
    long unusedA;
    long unusedB;
    Pos sp20;
    Pos sp28;
    SVECTOR rot;
    VECTOR trans;
    MATRIX m;
    SVECTOR sp6B = {.vx = 0, .vy = 20, .vz = 0};
    Entity* tempEntity;
    Primitive* prim;
    s32 posX;
    s32 posY;
    s32 primIndex;
    s32 var_s3;

    tempEntity = self - 3;
    self->facingLeft = tempEntity->facingLeft;
    self->rotate = (self->ext.plateLordUnknown.unkB0 - ROT(180)) / 2;
    switch (self->step) {
    case 0:
        InitializeEntity(g_EInitPlateLord);
        self->hitPoints = 0x7FFE;
        self->animCurFrame = 2;
        self->ext.plateLordUnknown.unkB0 = ROT(168.75);
        self->zPriority = 0xB1;
        self->hitboxWidth = 0xC;
        self->hitboxHeight = 0xC;
        self->drawFlags |= ENTITY_ROTATE;

        primIndex = g_api.AllocPrimitives(PRIM_LINE_G2, 2);
        if (primIndex == -1) {
            DestroyEntity(self);
            return;
        }
        self->flags |= FLAG_HAS_PRIMS;
        self->primIndex = primIndex;
        prim = &g_PrimBuf[primIndex];
        self->ext.prim = prim;
        prim->type = PRIM_GT4;
        prim->tpage = 0x13;
        prim->clut = PAL_PLATE_LORD_UNK2;
        setUV4(prim, 0x37, 0x0, 0x3F, 0x5, 0x30, 0xC, 0x38, 0x10);
        setRGB0(prim, 0x40, 0xC0, 0x80);
        setRGB1(prim, 0x80, 0x0, 0xC0);
        prim->priority = 0xB4;
        prim->drawMode = DRAW_UNK02;

        prim = prim->next;
        prim->type = PRIM_GT4;
        prim->tpage = 0x13;
        prim->clut = PAL_PLATE_LORD_UNK1;
        setUV4(prim, 0x50, 0x10, 0x50, 0x20, 0x38, 0x10, 0x38, 0x20);
        prim->priority = 0xB5;
        prim->drawMode = DRAW_UNK02;
        prim->r0 = 0x80;
        prim->g1 = 0x80;
        prim->b2 = 0x80;
        prim->r3 = prim->g3 = 0xC0;

        self->ext.plateLordUnknown.unk80 = ROT(33.75);
        self->ext.plateLordUnknown.unk94 = ROT(22.5);
        self->ext.plateLordUnknown.unk96 = ROT(-39.375);
        self->ext.plateLordUnknown.unk98 = ROT(45);
        return;

    case 9:
        break;
    }
    tempEntity = self - 3;
    func_801CD78C((Pos*)&tempEntity->posX, 0x10,
                  self->ext.plateLordUnknown.unkB0, (Pos*)&self->posX);
    if (self->step == 9) {
        posX = rcos(self->ext.plateLordUnknown.unk82) * 0x10;
        if (self->facingLeft) {
            self->posX.val -= posX;
        } else {
            self->posX.val += posX;
        }
    }
    tempEntity = self + 4;
    func_801CD78C((Pos*)&self->posX, 8, self->ext.plateLordUnknown.unk98,
                  (Pos*)&tempEntity->posX);
    tempEntity->rotate = self->ext.plateLordUnknown.unk98 - ROT(45);
    tempEntity = self + 5;
    func_801CD78C((Pos*)&self->posX, 0x12, self->ext.plateLordUnknown.unk98,
                  (Pos*)&tempEntity->posX);
    tempEntity->rotate = self->ext.plateLordUnknown.unk98 - ROT(45);
    posX = self->posX.i.hi;
    posY = self->posY.i.hi;

    if (self->animCurFrame == 2) {
        posY -= 6;
        if (self->facingLeft) {
            posX -= 12;
        } else {
            posX += 12;
        }
    } else {
        posY += 6;
        if (self->facingLeft) {
            posX += 10;
        } else {
            posX -= 10;
        }
    }
    sp20.x.i.hi = posX;
    sp20.y.i.hi = posY;
    if (self->step == 9) {
        posX = (rcos(self->ext.plateLordUnknown.unk82) << 1) << 4;
        if (self->facingLeft) {
            sp20.x.val -= posX;
        } else {
            sp20.x.val += posX;
        }
    }
    tempEntity = self + 3;
    tempEntity->posX.val = sp20.x.val;
    tempEntity->posY.val = sp20.y.val;
    func_801CD78C(&sp20, -3, self->ext.plateLordUnknown.unk96, &sp20);
    rot.vx = self->ext.plateLordUnknown.unk94;
    rot.vy = 0;
    rot.vz = self->ext.plateLordUnknown.unk96;
    if (self->step == 9) {
        rot.vz += rcos(self->ext.plateLordUnknown.unk82) >> 5;
    }
    RotMatrix(&D_us_80181F98, &m);
    RotMatrixX(rot.vx, &m);
    RotMatrixZ(rot.vz, &m);
    if (self->facingLeft) {
        RotMatrixY(ROT(180), &m);
    }
    trans.vx = 0;
    trans.vy = 0;
    trans.vz = 0x400;
    TransMatrix(&m, &trans);
    SetRotMatrix(&m);
    SetTransMatrix(&m);
    SetGeomScreen(0x400);
    SetGeomOffset(sp20.x.i.hi, sp20.y.i.hi);
    RotTransPers(&sp6B, &sp70, &unusedA, &unusedB);
    sp28.x.i.hi = sp70 & 0xFFFF;
    sp28.y.i.hi = sp70 >> 0x10;
    prim = self->ext.prim;
    func_us_801D2424(&sp20, rot.vz, 4, &sp28, rot.vz, 4, prim);
    if (self->palette & PAL_UNK_FLAG) {
        prim->clut = self->palette & 0xFFF;
    } else {
        prim->clut = PAL_PLATE_LORD_UNK2;
    }
    prim = prim->next;
    sp20.x.i.hi = sp70 & 0xFFFF;
    sp20.y.i.hi = sp70 >> 0x10;
    func_801CD78C(&sp20, -5, self->ext.plateLordUnknown.unk80, &sp20);
    tempEntity = self + 2;
    func_801CD78C(
        &sp20, 0x1A, self->ext.plateLordUnknown.unk80, (Pos*)&tempEntity->posX);
    tempEntity->rotate = self->ext.plateLordUnknown.unk80 - ROT(135);
    var_s3 = self->ext.plateLordUnknown.unk80;
    if (self->step == 9) {
        posX = rcos(self->ext.plateLordUnknown.unk82) * 0xD0;
        if (self->facingLeft) {
            tempEntity->posX.val -= posX;
        } else {
            tempEntity->posX.val += posX;
        }
        self->ext.plateLordUnknown.unk82 += self->ext.plateLordUnknown.unk84;
        self->ext.plateLordUnknown.unk86 += 2;
        self->ext.plateLordUnknown.unk84 =
            (self->ext.plateLordUnknown.unk86 << 7) >> 8;
        var_s3 += ratan2(posX >> 0x10, 0x1A);
    }
    func_us_801D2424(
        &sp20, var_s3, 8, (Pos*)&tempEntity->posX, var_s3, 8, prim);

    if (self->palette & PAL_UNK_FLAG) {
        prim->clut = self->palette & 0xFFF;
    } else {
        prim->clut = PAL_PLATE_LORD_UNK1;
    }
}

void func_us_801D4AA4(Entity* self) {
    Entity* tempEntity;

    tempEntity = self - 4;
    self->facingLeft = tempEntity->facingLeft;
    switch (self->step) {
    case 0:
        InitializeEntity(g_EInitPlateLord);
        self->hitPoints = 0x7FFE;
        self->hitboxWidth = 0;
        self->hitboxHeight = 0;
        switch (self->params) {
        case 0:
            self->zPriority = 0xB2;
            self->animCurFrame = 1;
            self->ext.plateLordUnknown.unkB0 = ROT(180);
            self->step = 1;
            self->hitboxWidth = 4;
            self->hitboxHeight = 4;
            break;
        case 1:
            self->zPriority = 0xB5;
            self->animCurFrame = 4;
            self->step = 2;
            break;
        case 2:
            self->animCurFrame = 9;
            self->zPriority = 0xAD;
            self->step = 3;
            self->drawFlags |= ENTITY_ROTATE;
            break;
        case 3:
            self->animCurFrame = 10;
            self->zPriority = 0xAE;
            self->step = 4;
            self->drawFlags |= ENTITY_ROTATE;
            break;
        }
        break;
    case 1:
        tempEntity = self - 1;
        if (tempEntity->animCurFrame == 18) {
            self->animCurFrame = 17;
            self->ext.plateLordUnknown.unkB0 = ROT(135);
        } else {
            self->animCurFrame = 1;
            self->ext.plateLordUnknown.unkB0 = ROT(180);
        }
        func_801CD78C((Pos*)&tempEntity->posX, 0xE,
                      self->ext.plateLordUnknown.unkB0, (Pos*)&self->posX);
        break;
    case 2:
        tempEntity = self - 6;
        switch (tempEntity->step) {
        case 9:
            self->animCurFrame = 4;
            self->zPriority = 0xB3;
            break;
        case 11:
            self->animCurFrame = 19;
            self->zPriority = 0xB5;
            break;
        default:
            self->animCurFrame = 4;
            self->zPriority = 0xB3;
            break;
        }
        break;
    case 3:
        tempEntity = self - 1;
        if (tempEntity->animCurFrame == 18) {
            self->animCurFrame = 0;
        } else {
            self->animCurFrame = 9;
        }
        break;
    case 4:
        tempEntity = self - 8;
        switch (tempEntity->step) {
        case 11:
            self->animCurFrame = 10;
            break;
        case 9:
            self->animCurFrame = 24;
            break;
        default:
            self->animCurFrame = 10;
            break;
        }
        tempEntity = self - 1;
        if (tempEntity->animCurFrame == 18) {
            self->animCurFrame = 0;
        }
        break;
    }
}

void func_us_801D4CAC(Entity* self) {
    Collider collider;
    Entity* tempEntity;
    s32 x;
    s32 y;

    tempEntity = self - 9;
    self->facingLeft = tempEntity->facingLeft;
    switch (self->step) {
    case 0:
        InitializeEntity(D_us_80180B90);
        self->animCurFrame = 22;
        self->zPriority = 0xB7;
        self->hitPoints = 0x7FFF;
        self->ext.plateLordUnknown.unkAC =
            self->posY.i.hi + g_Tilemap.scrollY.i.hi;
        break;
    case 1:
        break;
    case 16:
        self->posY.val += self->velocityY;
        self->velocityY += FIX(0.375);
        y = self->posY.i.hi + g_Tilemap.scrollY.i.hi;
        if (y >= self->ext.plateLordUnknown.unkAC) {
            x = self->posX.i.hi;
            y = self->posY.i.hi + 0xA;
            g_api.CheckCollision(x, y, &collider, 0);
            if (collider.effects & EFFECT_SOLID) {
                self->velocityY = -self->velocityY / 2;
                self->posY.i.hi += collider.unk18;
                self->ext.plateLordUnknown.unk80++;
            }
            if (self->ext.plateLordUnknown.unk80 > 3) {
                self->step = 17;
            }
        }
        break;
    case 17:
        self->hitboxState = HITBOX_INACTIVE;
        self->flags |= FLAG_DESTROY_IF_OUT_OF_CAMERA |
                       FLAG_DESTROY_IF_BARELY_OUT_OF_CAMERA;
        break;
    }
}
