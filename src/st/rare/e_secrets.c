...
static s16 breakableFloorTilesCollision[] = {
    0x0476,
    0x0477,
    0x0478,
    0x0479,
...
void EntityBreakableFloorSecret(Entity* self) {
    ...
    case 0:
        ...
        pSrcTile = breakableFloorTilesCollision;
        if (g_CastleFlags[RARE_SECRET_FLOOR_OPEN]) {
    ...
    case 2:
        ...
        if (self->ext.breakable.breakCount == 3) {
            g_api.PlaySfx(SFX_WALL_DEBRIS_B);
            g_CastleFlags[RARE_SECRET_FLOOR_OPEN] = true;
            g_api.RevealSecretPassageAtPlayerPositionOnMap(
                RARE_SECRET_FLOOR_OPEN);
